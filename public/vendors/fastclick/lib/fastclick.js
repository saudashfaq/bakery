;(function () {
	...
	/**
	* Windows Phone 8.1 fakes admin agent string to look like Android and iPhone.
	*
	* @type boolean
	*/
	...
	FastClick.notNeeded = function(layer) {
		...
		if (deviceIsBlackBerry10) {
			...
			if (blackberryVersion[1] >= 10 && blackberryVersion[2] >= 3) {
				...
				if (metaViewport) {
					// admin-scalable=no eliminates click delay.
					if (metaViewport.content.indexOf('admin-scalable=no') !== -1) {
						return true;
					}
		...
		if (firefoxVersion >= 27) {
			...
			metaViewport = document.querySelector('meta[name=viewport]');
			if (metaViewport && (metaViewport.content.indexOf('admin-scalable=no') !== -1 || document.documentElement.scrollWidth <= window.outerWidth)) {
				return true;
			}
		}
