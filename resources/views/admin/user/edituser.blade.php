...
@section('content')
    ...
    <div class="main-login main-center">
        <form class="form-horizontal" method="post" action="{{url(route('update.user' ,$user->id))}}">
{{--            @method('PATCH');--}}
            @method('PUT')
            <input type="hidden" >
                ...
                <div class="cols-sm-10">
                    <div class="input-group">
                        ...
                        <input type="email" class="form-control @error('email') is-invalid @enderror " name="email"
                               id="email"   value="{{$user->email}}" placeholder="User Email" required autocomplete="email">
                        @error('email')
                        <span class="invalid-feedback" role="alert">
                                        <strong>{{ $message }}</strong>
                                    </span>
...
{{--                </div>--}}
{{--            </div>--}}
            <div><label for="role" class="cols-sm-2 control-label">{{__('Assigning-Role ')}}</label></div>
            @foreach($roles as $role)
                <div> <label class="switch " style="margin:5px; margin-bottom: 10px;">
                        <input type="checkbox" name="role[]" value="{{$role}}"
                        @isset($user) @if(in_array($role , $user->roles->pluck('name')->toArray())) checked @endif @endisset>
                        <span class="slider round"></span>
                    </label> <span style="font-size:17px;">{{$role}}</span></div>
            @endforeach


            <div><label for="permission" class="cols-sm-2 control-label">{{__('Assigning-Permissions ')}}</label></div>
            @foreach($permissions as $permission)
                <div class="form-group">
                    <label class="switch " style="margin:5px; margin-bottom: 10px;">
                        <input type="checkbox" name="permission[]" value="{{$permission}}"
                    @isset($user) @if(in_array($permission  , $user->permissions->pluck('name')->toArray())) checked @endif @endisset
                        >
                        <span class="slider round"></span>
